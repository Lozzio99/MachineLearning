...
def fmincg(f, X_original, length=100, debug=False):  # , P1, P2, P3, P4, P5):
    ...
    while i < abs(length):  # while not finished
        ...
        if success:  # if line search succeeded
            ...
            if debug:
                print(S, i, '| Cost: ', f1)
            s = (np.dot(np.transpose(df2), df2) - np.dot(np.transpose(df1), df2)) / np.dot(np.transpose(df1),
                                                                                           df1) * s - df2  # Polack-Ribiere direction
            # swap derivatives
            tmp = df1
            df1 = df2
            df2 = tmp
