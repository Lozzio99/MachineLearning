...
def nnCostFunction(nn_params, input_layer_size, hidden_layer_size, num_labels, X, y, lambda_value):
    ...
    out_standard = [x + 1 for x in range(0, num_labels)]  # [1 , 2 , 3]
    out: Callable[[int], list[int]] = lambda x: [1 if e == x else 0 for e in out_standard]  # out(2) -> [ 0, 1, 0]
    ...
    for i in range(0, m):
        a1 = X[i]
        z2 = np.dot(np.append([1], a1), theta1)
        a2 = sigmoid(z2)
        z3 = np.dot(np.append([1], a2), theta2)
        a3 = sigmoid(z3)
        d3 = a3 - out(y[i])
        d2 = np.dot(theta2, d3)[1:] * sigmoidGradient(z2)
        Theta2_grad = np.add(Theta2_grad, np.outer(np.append([1], a2), d3).T)
        Theta1_grad = np.add(Theta1_grad, np.outer(np.append([1], a1), d2).T)

    Theta1_grad[:, 1:] = np.add(Theta1_grad[:, 1:], Theta1[:, 1:]*lambda_value)
    Theta2_grad[:, 1:] = np.add(Theta2_grad[:, 1:], Theta2[:, 1:]*lambda_value)
    Theta1_grad = np.divide(Theta1_grad, m)
    Theta2_grad = np.divide(Theta2_grad, m)
